...
final_content=$(printf "set(TR_PUBLIC_INCLUDE_DIR \${CMAKE_CURRENT_SOURCE_DIR}/$PUBLIC_INCLUDE_DIR CACHE STRING \"Include Directory for TurnipRenderer\")\nset(TR_PRIVATE_INCLUDE_DIR \${CMAKE_CURRENT_SOURCE_DIR}/$PRIVATE_INCLUDE_DIR)\nset(TR_HEADER_FILES $header_files)\nset(TR_SRC_FILES $c_files)")
